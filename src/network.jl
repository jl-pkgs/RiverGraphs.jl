# Wflow model arrays use the native PCRaster orientation; spatial rasters use `pcr_dir`.
const LDD_PIT = 5
const PCR_DIR = [
  CartesianIndex(-1, -1),
  CartesianIndex(0, -1),
...
"Return the source and destination node of each edge of a directed graph."
function adjacent_nodes_at_edge(graph::SimpleDiGraph{Int})
...
function adjacent_edges_at_node(graph::SimpleDiGraph{Int}, nodes_at_edge::NodesAtEdge)
  ...
  for node_idx in 1:nv(graph)
    push!(source_edges, findall(isequal(node_idx), nodes_at_edge.dst))
    push!(destination_edges, findall(isequal(node_idx), nodes_at_edge.src))
  end
  return (src=source_edges, dst=destination_edges)
end

"Add outlet vertices and edges for `pits` to a directed graph."
...
function NetworkRiver(
  ...
  river_values = ndims(river_location) == 2 ? river_location[network.indices] : river_location
